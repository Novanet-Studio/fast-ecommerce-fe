...
import { loadScript, type OrderResponseBody, type PayPalNamespace } from "@paypal/paypal-js";
...
const sendInvoiceEmail = async (order: OrderResponseBody, items: CartItem[]) => {
  try {
    let emailContent = '';
    const productItems = [];
    ...
    items.forEach((item) => {
      const finded = productsMail.value!.find((product: Product) => product.id === item.id) as Product;
...
const loadPaypal = async () => {
  ...
  try {
    await (paypal.value as PayPalNamespace).Buttons({
